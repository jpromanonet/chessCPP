...
bool Pawn::moveTo(Player& byPlayer, Square& to)
{
    ...
    else
    {
        ...
        valid = RestrictedPlace::moveTo(byPlayer, to);
...
bool Pawn::canMoveTo(Square& location) const
{
    ...
    {
        // ("forward" is "backward" relative to the board for black pieces)
        if(!isWhite())
        {
            translationX *= -1;
            translationY *= -1;
        ...
        // Valid move if !moved and moving 2 squares forward to unoccupied square along a clear vertical
        else if(!hadMoved() && translationY == 2 && translationX == 0 &&
